...
(function (scope) {
  ...
  scope.InfiniteScroller.prototype = {
    ...
    onScroll_: function () {
      ...
      var lastScreenItem = this.calculateAnchoredItem(
        this.anchorItem,
        this.scroller_.offsetHeight
      );
      if (delta < 0)
        this.fill(
          this.anchorItem.index - RUNWAY_ITEMS,
          lastScreenItem.index + RUNWAY_ITEMS_OPPOSITE
        );
      else
        this.fill(
          this.anchorItem.index - RUNWAY_ITEMS_OPPOSITE,
    ...
    calculateAnchoredItem: function (initialAnchor, delta) {
      ...
      if (delta < 0) {
        ...
        tombstones = Math.max(
          -i,
          Math.ceil(Math.min(delta, 0) / this.tombstoneSize_)
        );
      } else {
        while (
          delta > 0 &&
          i < this.items_.length &&
          this.items_[i].height &&
          this.items_[i].height < delta
        ) {
    ...
    attachContent: function () {
      ...
      for (i = 0; i < this.items_.length; i++) {
        ...
        if (this.items_[i].node) {
          if (this.items_[i].node.classList.contains("tombstone")) {
            ...
            this.tombstones_[this.tombstones_.length - 1].classList.add(
              "invisible"
            );
          } else {
      ...
      for (i = this.firstAttachedItem_; i < this.lastAttachedItem_; i++) {
        while (this.items_.length <= i) this.addItem_();
        if (this.items_[i].node) {
          // if it's a tombstone but we have data, replace it.
          if (
            this.items_[i].node.classList.contains("tombstone") &&
            this.items_[i].data
          ) {
            ...
            if (ANIMATION_DURATION_MS) {
              ...
              tombstoneAnimations[i] = [
                this.items_[i].node,
                this.items_[i].top - this.anchorScrollTop
              ];
            } else {
        ...
        var node = this.items_[i].data
          ? this.source_.render(this.items_[i].data, unusedNodes.pop())
          : this.getTombstone();
      ...
      for (var i in tombstoneAnimations) {
        ...
        this.items_[i].node.style.transform =
          "translateY(" +
          (this.anchorScrollTop + anim[1]) +
          "px) scale(" +
          this.tombstoneWidth_ / this.items_[i].width +
          ", " +
          this.tombstoneSize_ / this.items_[i].height +
          ")";
        // Call offsetTop on the nodes to be animated to force them to apply current transforms.
        this.items_[i].node.offsetTop;
        anim[0].offsetTop;
        this.items_[i].node.style.transition =
          "transform " + ANIMATION_DURATION_MS + "ms";
      }
      for (i = this.firstAttachedItem_; i < this.lastAttachedItem_; i++) {
        var anim = tombstoneAnimations[i];
        if (anim) {
          anim[0].style.transition =
            "transform " +
            ANIMATION_DURATION_MS +
            "ms, opacity " +
            ANIMATION_DURATION_MS +
            "ms";
          anim[0].style.transform =
            "translateY(" +
            curPos +
            "px) scale(" +
            this.items_[i].width / this.tombstoneWidth_ +
            ", " +
            this.items_[i].height / this.tombstoneSize_ +
            ")";
      ...
      this.scrollRunwayEnd_ = Math.max(
        this.scrollRunwayEnd_,
        curPos + SCROLL_RUNWAY
      );
      this.scrollRunway_.style.transform =
        "translate(0, " + this.scrollRunwayEnd_ + "px)";
    ...
    addContent: function (items) {
      ...
      for (var i = 0; i < items.length; i++) {
        if (this.items_.length <= this.loadedItems_) this.addItem_();
